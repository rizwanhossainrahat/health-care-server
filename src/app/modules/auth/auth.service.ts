...
const changePassword = async (user: any, payload: any) => {
    ...
    const hashedPassword: string = await bcrypt.hash(payload.newPassword, Number(10));
...
const forgotPassword = async (payload: { email: string }) => {
    ...
    const resetPassToken = jwtHelper.generateToken(
        ...
        config.resetPassToken as Secret,
        config.resetPassExpireIn as string
    )
...
const resetPassword = async (token: string, payload: { id: string, password: string }) => {
    ...
    const isValidToken = jwtHelper.verifyToken(token, config.resetPassToken as Secret)
